...
public class TerrainManager
{
    ...
    private Action<List<TerrainObjectData>> _makeGridUnvisible;
    private Action<List<TerrainObjectData>> _makeGridVisible;

    public TerrainManager(Transform player,TerrainAttributes terrainAttributes, 
        Action<List<TerrainObjectData>> makeGridUnvisible, Action<List<TerrainObjectData>> makeGridVisible)
    {
        _player = player;
        _gridSize = terrainAttributes.gridSize;
        _distancePlane = terrainAttributes.distancePlane;
        _makeGridUnvisible = makeGridUnvisible;
        _makeGridVisible = makeGridVisible;
    ...
    public void CheckVisibleTargets()
    {
        foreach ((int, int) visibleCoord in _visibleGrids)
        {
            ...
            {
                _makeGridUnvisible.Invoke(_terrainObjectGrids[(visibleCoord.Item1, visibleCoord.Item2)]);
            }
        ...
        foreach ((int, int) visibleCoord in _visibleGrids)
        {
            _makeGridVisible.Invoke(_terrainObjectGrids[(visibleCoord.Item1, visibleCoord.Item2)]);
        }
    }
